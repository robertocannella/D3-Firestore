...
const update = (data) => {
    ...
    rects
        ...
        .transition(t)
        .delay((d, i, n) => i * 300)
        .attrTween('width', widthTween) // Custom Tween (see below)
        .attr('y', d => yScale(d.orders))
        .attr('height', d => graphHeight - yScale(d.orders)) // for Y-Top Starting Offset
...
const widthTween = (data) => {
    let i = d3.interpolate(0, xScale.bandwidth()); // here we set to a range
    return function (t) { // return a function which takes a time ticker 't'
        // return the value from passsing the ticker into the interpolation.
        return i(t);
    }
}